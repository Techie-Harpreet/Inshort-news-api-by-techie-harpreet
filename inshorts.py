...
def getHindiNews(category):
    ...
    try:
        if category != 'all':
            htmlBody = requests.get(
                'https://www.inshorts.com/hi/read/' + category)
        else:
            htmlBody = requests.get('https://www.inshorts.com/hi/read/')

    except requests.exceptions.RequestException as e:
        newsDictionaryHindi['success'] = False
        newsDictionaryHindi['error'] = str(e.message)
        return newsDictionaryHindi
    ...
    for i in elements:
        ...
        newsDictionaryHindi['data'].append(newsObject)
